...
export default function Home() {
  ...
  return (
    <main>
      ...
      <button onClick={()=>{setCartDisplay(!cartDisplay)}} className="p-3 rounded-3xl bg-blue-600 text-white">장바구니 {cartDisplay ? '닫기' : '보기'}</button>
      {cartDisplay && <Cart />}
      </article>
      <article className="w-1/2 text-center">
      <button onClick={()=>{setOrderDisplay(!orderDisplay)}} className="p-3 rounded-3xl bg-blue-600 text-white">주문 내역 {orderDisplay ? '닫기' : '보기'}</button>
